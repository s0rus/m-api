...
async function setupStreamNotifier() {
  ...
  const channel = await DiscordClient.getInstance().channels.fetch(
    env.NODE_ENV === "production"
      ? discordId.MAIN_CHANNEL_ID
      : discordId.TEST_CHANNEL_ID,
  );

  streamNotifierList.forEach(async (streamer) => {
    try {
      const onlineSub = listener.onStreamOnline(
        streamer.twitchId,
        async (e) => {
      ...
      logger.info(
        `[${streamer.twitchName} test command]: ${await onlineSub.getCliTestCommand()}`,
      );
    } catch (error) {
...
async function sendStreamNotifyMessage({
  ...
  const streamLinkButton = new ButtonBuilder()
    .setLabel("Oglądaj")
    .setURL(
      `https://www.twitch.tv/${streamerTwitchInfo.name ?? streamer.twitchName}`,
    )
  ...
  const streamInfoEmbed = new EmbedBuilder()
    ...
    .setDescription(
      `## ${streamerTwitchInfo.name ?? streamer.twitchName} odpalił streama!`,
    );
