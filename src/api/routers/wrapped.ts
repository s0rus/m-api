...
wrapped.get('/:userId', async (c) => {
  ...
  try {
    ...
    const globalMessageCountAllTime = globalMessageCountAggregation._sum.totalMessageCount ?? 0;

    const userMessageCountByYear = getYearlyMessageCount(user.aggregations) ?? [];
    const userMessageCountAllTime = user.totalMessageCount ?? 0;
    const userMessageCountThisYear = userMessageCountByYear.find((item) => item.name === '2024')?.value ?? 0;
    ...
    const globalMessageDayCountLeadersThisYear = await db.messageAggregation.findMany({
      where: {
        date: {
          endsWith: '2024',
        },
    ...
    const globalMessageDayCountLeadersAllTime = await db.messageAggregation.findMany({
      orderBy: {
        dayCount: 'desc',
      },
      include: {
    ...
    const userReplyCount = user.userWrapped?.statAggregation?.replyCount ?? null;
    const userAttachmentCount = user.userWrapped?.statAggregation?.attachmentCount ?? null;
    const userMentionCount = user.userWrapped?.statAggregation?.mentionCount ?? null;
    const userReactionCount = user.userWrapped?.statAggregation?.reactionCount ?? null;
      ...
      wrapped: {
        messages: {
          userSpecific: {
            ...
            userMessageCountByWeekday: getWeekdayMessageCount(user.aggregations),
          ...
          serverSpecific: {
            ...
            globalMessageCountLeadersThisYear: globalMessageCountLeadersThisYear.map((i) => ({
              totalMessageCount: typeof i.daycount === 'bigint' ? Number(i.daycount.toString()) : i.daycount,
              id: i.user.id,
            ...
            globalMessageDayCountThisYear: globalMessageDayCountThisYear.map((i) => ({
              date: i.date,
              dayCount: i._sum.dayCount,
            })),
            globalMessageDayCountAllTime: globalMessageDayCountAllTime.map((i) => ({
              date: i.date,
              dayCount: i._sum.dayCount,
            })),
          },
